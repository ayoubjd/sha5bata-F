...
interface EditorState {
  /** Project identifier in localStorage (null = new/unsaved). */
  id: string | null;
  projectName: string;
  dirty: boolean;
  scenes: Scene[];
  ...
  // project save/load
  loadProject: (project: ProjectData, projectId?: string) => void;
  newProject: () => void;
}
...
export const useEditor = create<EditorState>((rawSet, get) => {
  ...
  const set: typeof rawSet = (partial) => {
    ...
    if (!get().dirty) rawSet({ dirty: true });
  ...
  loadProject: (project, projectId) => {
    ...
    rawSet({
      ...
      audio: project.audio,
      video: project.video,
      canvasMode: project.canvasMode ?? "landscape",
      customFonts: project.customFonts ?? [],
      customBackgrounds: project.customBackgrounds ?? [],
      sfxVolume: project.sfxVolume ?? 0.85,
      showQuill: project.showQuill ?? true,
